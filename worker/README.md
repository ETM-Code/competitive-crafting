...
- Three-second countdown, then crafting, then five-second reveal/intermission. The connected current host can send `skipReveal` with the current `roundId`: **Next round** starts the usual three-second countdown early, while **Show results** ends the final reveal immediately. The server rejects non-host, disconnected, stale-round and expired-reveal requests; skipping preserves awarded scores and round history. There is no Memory mode or preview phase. Countdown snapshots contain no target.
